...
function init(request) {
  try {
    ...
    var transcellToken = personium.getTranscellToken(
      params,
      appTokenUser.access_token
    );

    var extUrl = params.p_target;
    var appTokenExt = personium.getAppToken(params.p_target); // External cell (cell to be accessed)
    var token = personium.getProtectedBoxAccessToken4ExtCell(
      extUrl,
      transcellToken.access_token,
      appTokenExt.access_token
    );
